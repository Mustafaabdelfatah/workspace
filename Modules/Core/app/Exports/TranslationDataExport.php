...
class TranslationDataExport implements FromCollection,WithMapping, WithHeadings, ShouldAutoSize, WithStyles
{
    protected $data;
    ...
    public function collection()
    {
        ...
        if (!empty($this->searchKey)) {
            ...
            $q->where(function ($subQuery) {
                ...
                $subQuery->where('key', 'LIKE', "%{$this->searchKey}%")
                    ->orWhere('module', 'LIKE', "%{$this->searchKey}%")
                    ->orWhere('phrase', 'LIKE', "%{$this->searchKey}%");
            });
        }
    ...
    { 
        ...
        return [
            $item->module,
            $item->key,
            $item->getTranslation('phrase','en'),
            $item->getTranslation('phrase','ar')
        ];
